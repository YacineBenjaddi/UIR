...
class _InfoPageState extends State<InfoPage> {
  ...
  var id_loisir,nom,description,latitude,longitude,id_photo,nomphoto,capacite;
 ...
 /* Future<List<Loisir>> _getOneLoisir() async {
    ...
    if (dataloisir.length > 0) {
      setState(() {
        ...
        capacite = dataloisir[0]['capacite'];
      });
    }
    return dataloisir;
  }*/

  Future<List<Loisir>> getOneLoisir() async {
    final result = await Services.getOneLoisir(id_loisir);

    var dataloisir = json.decode(result);
  ...
  Future<List<Photo>> _getPhotos() async {
    ...
    final result = await Services.getPhotos(id_loisir);
  ...
  _reserver() {
    ...
    Navigator.push(
      ...
      MaterialPageRoute(builder: (context) => Picker(id_loisir:widget.id_loisir,capacite:capacite)),
    );
  }
}
